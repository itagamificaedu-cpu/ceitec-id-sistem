...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const totalAlunos = await db.get('SELECT COUNT(*) as total FROM alunos WHERE ativo = 1');
    const presentesHoje = await db.get("SELECT COUNT(*) as total FROM presencas WHERE data = ? AND status = 'presente'", [hoje]);
    const ausentesHoje = await db.get("SELECT COUNT(*) as total FROM presencas WHERE data = ? AND status = 'ausente'", [hoje]);
    ...
    const turmas = (await db.all('SELECT DISTINCT turma FROM alunos WHERE ativo = 1')).map(t => t.turma);
    const freqPorTurma = await Promise.all(turmas.map(async turma => {
      const alunosTurma = await db.get('SELECT COUNT(*) as total FROM alunos WHERE turma = ? AND ativo = 1', [turma]);
      const presentesTurma = await db.get("SELECT COUNT(*) as total FROM presencas WHERE data = ? AND status = 'presente' AND aluno_id IN (SELECT id FROM alunos WHERE turma = ?)", [hoje, turma]);
...
router.get('/turma', async (req, res) => {
  try {
    ...
    const alunos = await db.all('SELECT * FROM alunos WHERE turma = ? AND ativo = 1 ORDER BY nome', [turma]);
...
router.get('/frequencia-semanal', async (req, res) => {
  try {
    ...
    const resultado = await Promise.all(dias.map(async data => {
      const presentes = await db.get("SELECT COUNT(*) as total FROM presencas WHERE data = ? AND status = 'presente'", [data]);
      const ausentes = await db.get("SELECT COUNT(*) as total FROM presencas WHERE data = ? AND status = 'ausente'", [data]);
